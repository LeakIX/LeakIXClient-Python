...
class TestMustQuery:
    def test_serialize_with_country_field(self):
        field = CountryField("France")
        query = MustQuery(field)
        assert query.serialize() == "+country:France"

    def test_serialize_with_port_field(self):
        field = PortField(443)
        query = MustQuery(field)
        assert query.serialize() == "+port:443"

    def test_serialize_with_ip_field(self):
...
class TestRawQuery:
    def test_serialize_returns_raw_string(self):
        raw = '+plugin:HttpNTLM +country:"France"'
        query = RawQuery(raw)
        assert query.serialize() == raw

    def test_serialize_complex_query(self):
        raw = "+host:.be -port:22"
        query = RawQuery(raw)
        assert query.serialize() == raw
...
class TestCustomField:
    def test_serialize_without_operator(self):
        field = CustomField("test_value", "test_field")
        assert field.serialize() == "test_field:test_value"

    def test_serialize_with_equal_operator(self):
    ...
    def test_serialize_with_smaller_operator(self):
...
class TestAgeField:
    def test_serialize_with_age(self):
        field = AgeField(30)
...
class TestPluginField:
    def test_serialize_with_grafana_plugin(self):
...
class TestCountryField:
    def test_serialize_with_country(self):
        field = CountryField("US")
        assert field.serialize() == "country:US"

    def test_serialize_with_full_country_name(self):
        field = CountryField("France")
